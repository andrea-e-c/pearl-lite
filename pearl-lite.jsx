...
// ─── Canvas Scene ───
function DreamScene({ phase, opacity }) {
  ...
  const draw = useCallback((ctx, w, h, t) => {
    ...
    // Floating particles (firefly-like warmth)
    ...
    // Gentle grass-like details on shore
...
const PROMPT_STAGES = [
  {
    id: "welcome",
    text: null, // no question, just the opening moment
    delay: 3000,
  },
  {
    id: "what_want",
    ...
    branches: (input) => {
      ...
      if (/peace|calm|quiet|rest|still|serene|relax/.test(lower)) return "want_peace";
      if (/love|connect|relation|partner|friend|family|belong/.test(lower)) return "want_connection";
      if (/free|escape|travel|adventure|new|change|different/.test(lower)) return "want_freedom";
      if (/purpose|meaning|matter|impact|contribution|legacy/.test(lower)) return "want_purpose";
      if (/success|achieve|goal|win|money|career|grow/.test(lower)) return "want_success";
      if (/happy|joy|laugh|fun|light|play/.test(lower)) return "want_joy";
      if (/heal|recover|whole|better|strong|health/.test(lower)) return "want_healing";
      if (/know|understand|clarity|truth|answers|wisdom/.test(lower)) return "want_clarity";
      return "want_general";
    },
  },
  {
    id: "want_peace",
    text: "Peace. That's a deep thing to want. What is it that's been loud?",
    subtext: "What's been taking up the space where quiet should be?",
    next: "why_matters",
  },
  {
    id: "want_connection",
    text: "Connection. You want to be seen — really seen. What's missing right now?",
    subtext: "Where do you feel the gap?",
    next: "why_matters",
  },
  {
    id: "want_freedom",
    text: "Freedom. Something is holding you. What does the cage look like?",
    subtext: "What would you walk away from if you could?",
    next: "why_matters",
  },
  {
    id: "want_purpose",
    text: "Purpose. You want your days to mean something. What would that feel like?",
    subtext: "Not what it looks like — what it feels like.",
    next: "why_matters",
  },
  {
    id: "want_success",
    text: "You want to build something. To get somewhere. Where are you trying to go?",
    subtext: "And who are you when you arrive?",
    next: "why_matters",
  },
  {
    id: "want_joy",
    text: "Joy. Simple and honest. When was the last time you really felt it?",
    subtext: "Let yourself remember.",
    next: "why_matters",
  },
  {
    id: "want_healing",
    text: "Healing. Something needs to mend. What part of you is asking for care?",
    subtext: "You don't have to name it perfectly.",
    next: "why_matters",
  },
  {
    id: "want_clarity",
    text: "Clarity. The fog is thick, isn't it? What question won't leave you alone?",
    subtext: "The one that keeps circling back.",
    next: "why_matters",
  },
  {
    id: "want_general",
    text: "Sit with that for a moment. Why does that matter to you?",
    subtext: "Not the surface reason — the real one.",
    next: "why_matters",
  },
  {
    id: "why_matters",
    ...
    branches: (input) => {
      const lower = input.toLowerCase();
      if (/afraid|fear|scared|worry|anxious|lose/.test(lower)) return "fear_response";
      if (/love|care|someone|people|them|her|him|they/.test(lower)) return "love_response";
      if (/deserve|worth|enough|should|owe/.test(lower)) return "worth_response";
      if (/time|running out|short|urgent|wast/.test(lower)) return "time_response";
      if (/always|never|every|since|childhood|kid|young|growing/.test(lower)) return "pattern_response";
      return "depth_response";
    },
  },
  {
    id: "fear_response",
    text: "There's something you're afraid of losing — or never having. What does the fear protect?",
    subtext: "Fear usually guards what we love most.",
    next: "truth",
  },
  {
    id: "love_response",
    text: "This is about love, isn't it? For someone, for something, for who you could be.",
    subtext: "What does that love need from you today?",
    next: "truth",
  },
  {
    id: "worth_response",
    text: "You're questioning whether you're enough. But let me ask — who told you that you weren't?",
    subtext: "Whose voice is that, really?",
    next: "truth",
  },
  {
    id: "time_response",
    text: "Time feels heavy right now. Like the window is closing. But is it, truly?",
    subtext: "Or does it just feel that way?",
    next: "truth",
  },
  {
    id: "pattern_response",
    text: "This has been with you for a long time. You've carried it far. What would it feel like to set it down?",
    subtext: "Even just for today.",
    next: "truth",
  },
  {
    id: "depth_response",
    text: "You're going deeper. Good. What's the truest thing you can say about this?",
    subtext: "The thing you might not say out loud.",
    next: "truth",
  },
  {
    id: "truth",
    text: "Now — say it simply. What do you really, truly want?",
    subtext: "One sentence. Your truth.",
    next: "align",
  },
  {
    id: "align",
    text: "You know what you want. What's one thing you can do today to move toward it?",
    subtext: "Not everything. Just one thing. The smallest honest step.",
    next: "closing",
  },
  {
    id: "closing",
...
// ─── Typewriter Effect ───
function TypewriterText({ text, speed = 40, onDone }) {
...
export default function PearlLite() {
  const [phase, setPhase] = useState("arriving"); // arriving, sitting, journey, waking, summary
  ...
  // Phase: Arriving — fade in the scene
  useEffect(() => {
    if (phase === "arriving") {
  ...
  // Phase: Sitting — show opening text
  useEffect(() => {
    if (phase === "sitting") {
  ...
  // Phase: Journey — manage typing and input visibility
  useEffect(() => {
  ...
  const handleSubmit = () => {
    ...
    setConversationLog((prev) => [
      ...prev,
      { question: stage.text, answer, stageId },
    ]);
    ...
    if (stage.branches) {
      ...
      // This was the alignment question — trigger waking
  ...
  // Extract the "truth" and "action" from conversation
  const truthEntry = conversationLog.find((c) => c.stageId === "truth");
  const alignEntry = conversationLog.find((c) => c.stageId === "align");
  const firstWant = conversationLog.find((c) => c.stageId === "what_want");

  return (
    <div style={{ width: "100vw", height: "100vh", overflow: "hidden", background: "#000" }}>
      <DreamScene phase={phase} opacity={sceneOpacity} />
        ...
        >
          ...
          >
              ...
              <div style={{ marginBottom: "2.5rem" }}>
                ...
                <h2
                  style={{
                    ...
                    fontWeight: 400,
                    lineHeight: 1.5,
                    textShadow: "0 2px 20px rgba(253,203,110,0.3)",
                    margin: 0,
                  }}
                >
                  "{truthEntry.answer}"
            ...
            <button
              ...
              onMouseEnter={(e) => {
                e.target.style.background = "rgba(253,203,110,0.08)";
                e.target.style.borderColor = "rgba(253,203,110,0.5)";
              }}
              onMouseLeave={(e) => {
                e.target.style.background = "transparent";
                e.target.style.borderColor = "rgba(253,203,110,0.3)";
              }}
            >
              dream again
            </button>
